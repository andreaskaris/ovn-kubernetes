...
var _ = Describe("ACL Logging for EgressFirewall", func() {
	const (
		...
		namespacePrefix         = "acl-logging-egressfw"
	...
	BeforeEach(func() {
		...
		err = makeEgressFirewall(fr, nsName)
		Expect(err).NotTo(HaveOccurred())

		By("creating a pod running agnhost netexec")
		cmd := []string{"/bin/bash", "-c", "/agnhost netexec --http-port 8000"}
		pod := newAgnhostPod("pod", cmd...)
...
func makeEgressFirewall(f *framework.Framework, ns string) error {
...
`)
	...
	_, err := framework.RunKubectl(f.Namespace.Name, "create", "-f", egressFirewallYaml)
	return err
}
...
func pokeExternalHost(fr *framework.Framework, pokePod *v1.Pod, dstIp string, dstPort int) {
	By("sending traffic outside to test triggering ACL logging")
	framework.Logf(
		"Poke destination %s:%d from pod %s (on node %s)",
		dstIp,
		dstPort,
		pokePod.GetName(),
		pokePod.Spec.NodeName,
	)
	pokeExternalHostFromPod(fr, pokePod.GetName(), dstIp, dstPort)
}
