...
func ensureLocalRaftServerID(db string) error {
	...
	for _, member := range members {
		if len(member) < 2 {
			return fmt.Errorf("unable to find server id submatch in %s", member)
		}
...
func ensureClusterRaftMembership(db string, kclient kube.Interface) error {
	...
	for _, member := range members {
		if len(member) < 3 {
			return fmt.Errorf("unable to find parse member: %s", member)
		}
		matchedServer := strings.Split(member[2], ":")
		if len(matchedServer) < 3 {
			return fmt.Errorf("unable to parse address portion of the member entry: %s", matchedServer)
		}
		memberFound := false
