...
func (n *OvnNode) WriteCurrentEncapIP() error {
	...
	cmd := []string{
		...
		"Open_vSwitch",
		".",
		fmt.Sprintf("external_ids:ovn-encap-ip=%s", currentEncapIP),
	}
...
func (n *OvnNode) validateVTEPInterfaceMTU() error {
	...
	ovnEncapIP := net.ParseIP(currentEncapIP)
	if ovnEncapIP == nil {
		return fmt.Errorf("the set OVN Encap IP is invalid: (%s)", currentEncapIP)
	}
